...
@WebServlet(urlPatterns = {"/home", "/product", "/login","/user-rating", "/book-author"})
...
//	    }
	...
	private void login(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if (count == 1) {
			...
			response.sendRedirect("/Website_QuaTrinh/login");
		}
	}
	private void doGetBookAuthor(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (user != null) {
			...
			List<BooksEntity> book = bookAuthorDAO.findBookByBookAuthor(user.getId());
			List<AuthorEntity> author= bookAuthorDAO.findAuthorByBooksAuthor(user.getId());
